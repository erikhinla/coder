...
// ErrReinitializeAgentRoutines is a sentinel error used to indicate that the agent should refresh its configuration.
// It is primarily used in scenarios like prebuilt workspace claims, where parts of the agent definition have changed
// and the agent must reinitialize using the new configuration. Conceptually similar to receiving a SIGHUP signal.
var ErrReinitializeAgentRoutines = xerrors.New("reinitialize agent routines")
...
func (a *agent) run() (retErr error) {
	...
	for {
		...
		connMan.startAgentAPI("wait for reinitialization", gracefulShutdownBehaviorStop,
			func(ctx context.Context, aAPI proto.DRPCAgentClient24) error {
				_, err := aAPI.WaitForReinitialize(ctx, &proto.WaitForReinitializeRequest{})
				if err != nil {
					// TODO: ensure that handling this error does not break BC
					return xerrors.Errorf("wait for reinitialization: %w", err)
				}
				return ErrReinitializeAgentRoutines
			},
		)
		...
		err = connMan.wait()
		if errors.Is(err, ErrReinitializeAgentRoutines) {
			continue
		}
		return err
	}
